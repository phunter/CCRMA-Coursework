...
public class Slugscape_Visuals extends PApplet {
...
// View Layout Things
float bottom_frame_height = 0.618033988f; // Golden Ratio, just cuz
...
public void setup() {
  ...
  upper_width = width / 3;
  upper_height = (int) ((1.0f - bottom_frame_height) * height);
...
public void drawRecord(float pitch, float amplitude, int machine_num, int frame_num) {
  
  int radius = (int) map(amplitude, 0.0f, 1.0f, 0.0f, 20.0f);
...
public void drawAveraging(float pitch, float amplitude, int machine_num, int frame_num) {
  ...
  int c = color(0, 0, 0); // initialize to make processing happy
  if (machine_num == 0) {
    c = color(150, 20, 20);
  }
  else if (machine_num == 1) {
...
public void mousePressed() { // for testing
  ...
  if (random(1) > .15f) { // most of the time send envelope msg
...
public void oscEvent(OscMessage theOscMessage) {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--hide-stop", "Slugscape_Visuals" });
  }
}
